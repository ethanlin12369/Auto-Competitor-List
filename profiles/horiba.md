# HORIBA

*Auto-generated profile — last updated 2026-07-29.*
Appears on: SERS, TECH. Tracked page(s): https://www.horiba.com/usa/scientific/products/detail/action/show/Product/sers-substrates-1-6574/

## SERS list details

| Field | Value |
|---|---|
| Category | Instrument vendor |
| Product / Brand | SERS substrates (Silmeco-based) |
| Tracked URL | https://www.horiba.com/usa/scientific/products/detail/action/show/Product/sers-substrates-1-6574/ |
| Pricing (baseline Jul 2026) | On request (RFI form) |
| Pricing Model & MOQ | Quote-based; bundled with Raman systems |
| Primary Markets | Pharma, biotech, food, research |
| Distribution | Global direct + distributors; integrated with HORIBA Raman instruments |
| Funding / Investors | Public company (TSE:6856) |
| Certifications | Corporate ISO suite |
| USP / Positioning | One-stop Raman vendor: substrate + instrument + service + global brand |
| Notes | Controls the channel; resells Silmeco - potential OEM partner rather than head-to-head |
| Prices Seen on Page | (none listed) |
| Stock Alerts | - |
| Last Change Detected | — |
| Last Checked | 2026-07-29 |
| Page Status | OK |

## TECH list details

| Field | Value |
|---|---|
| Nanoparticles | Selling Silmeco (resold) |
| Platform Type | - |
| Price per Substrate (EUR) | - |
| Enhancement Factor (EF) | - |
| Patents | - |
| Technical Notes | Resells Silmeco substrates; one-stop Raman vendor |
| Website | https://www.horiba.com/usa/scientific/products/detail/action/show/Product/sers-substrates-1-6574/ |
| Prices Seen on Page | (none listed) |
| Stock Alerts | - |
| Last Change Detected | — |
| Last Checked | 2026-07-29 |
| Page Status | OK |
